...
local showMBRadar = imgui.new.bool(true)
...
-- ESP Focus Mode (moneybag/event location highlighting)
local espFocusPosition = nil
local espFocusTime = 0
local ESP_FOCUS_DURATION = 60 -- Auto-clear after 60s
...
local function findBestMatchPosition(searchTerms)
    ...
    if highestScore >= 20 then
        ...
        if bestMatch then
            local posName = bestMatch.name:lower()
...
local function renderPositionESP()
    ...
    -- Check ESP Focus (moneybag/event mode — only show target location)
    local focusDist = nil
    if espFocusPosition then
        focusDist = calculateDistance(playerX, playerY, playerZ, espFocusPosition.x, espFocusPosition.y, espFocusPosition.z)
        local elapsed = os.clock() - espFocusTime
        
        -- Auto-clear: arrived within 10m or timeout
        if focusDist < 10 or elapsed > ESP_FOCUS_DURATION then
    ...
    if espFocusPosition then
        -- FOCUS MODE: render only the focused position
        repeat
        ...
        local alpha = math.floor(0xAA + pulse * 0x55)
        local focusColor = (alpha * 0x1000000) + 0xFFA500
        local focusLineColor = (math.floor(0x66 * pulse) * 0x1000000) + 0xFFA500
        local displayName = espFocusPosition.name or "FOCUS"
        local distText = string.format("[%.0fm]", focusDist)
        
        if sx > 0 and sx < scrW and sy > 0 and sy < scrH then
            foundCount = 1
            -- Thick pulsing line
            renderDrawLine(scrW / 2, scrH, sx, sy, 2.5, focusLineColor)
            renderDrawLine(scrW / 2, scrH, sx, sy, 1.5, focusColor)
            renderDrawPolygon(sx - 5, sy - 5, 10, 10, 6, 0, focusColor)
            if espFont then
                renderFontDrawText(espFont, "~ " .. displayName, sx + 8, sy - 16, focusColor)
                renderFontDrawText(espFont, distText, sx + 8, sy + 4, 0xFFFFFFFF)
            end
        else
            local edgeX, edgeY
            if sx < 0 then
                edgeX = 10
                edgeY = math.max(50, math.min(scrH - 10, sy * (scrW / (scrW - sx))))
            elseif sx > scrW then
                edgeX = scrW - 10
                edgeY = math.max(50, math.min(scrH - 10, sy * (scrW / sx)))
            else
                edgeX = math.max(10, math.min(scrW - 10, sx))
                if sy < 0 then edgeY = 10 else edgeY = scrH - 10 end
            end
            if not edgeY then
                if sy < 0 then
                    edgeY = 10
                    edgeX = math.max(10, math.min(scrW - 10, sx * (scrH / (scrH - sy))))
                elseif sy > scrH then
                    edgeY = scrH - 10
                    edgeX = math.max(10, math.min(scrW - 10, sx * (scrH / sy)))
                end
            end
            if edgeX and edgeY then
                renderDrawLine(scrW / 2, scrH, edgeX, edgeY, 2.0, focusLineColor)
                if espFont then
                    renderFontDrawText(espFont, "~ " .. displayName, edgeX - 35, edgeY - 10, focusColor)
                    renderFontDrawText(espFont, distText, edgeX - 35, edgeY + 6, 0xFFFFFFFF)
                end
            end
        end
        until true
        
        printStringNow(string.format("~y~FOCUS: ~w~%s ~r~~h~(%.0fm)", espFocusPosition.name or "?", focusDist), 500)
        return
    end
    
    -- No focus active — show waiting message
    if showESP[0] then
        printStringNow("~y~ESP: waiting for hint...", 500)
    end
end
...
local function renderMenu()
    ...
    imgui.SameLine()
    local radarColor = showMBRadar[0] and imgui.ImVec4(0.3, 0.6, 0.9, 1.0) or imgui.ImVec4(0.5, 0.5, 0.5, 0.6)
    imgui.PushStyleColor(imgui.Col.Button, radarColor)
    if imgui.Button(showMBRadar[0] and "Radar: ON" or "Radar: OFF", imgui.ImVec2(90, 25)) then
        showMBRadar[0] = not showMBRadar[0]
    end
    imgui.PopStyleColor()
    if imgui.IsItemHovered() then
        imgui.SetTooltip("Show mini radar for moneybag positions (bottom-right)")
    end
    ...
    for _, entry in ipairs(sortedPositions) do
        ...
        local headerText = string.format("%s%d. %s", favIcon, i, posName)
...
function renderMoneybagESP()
    ...
    if not next(moneyBags) then return end
...
function renderMoneybagRadar()
    if not showMoneybags[0] or not showMBRadar[0] then return end

    local px, py, pz = getCharCoordinates(PLAYER_PED)
    local scrW, scrH = getScreenResolution()
    local cx, cy = scrW - 120, scrH - 120
    local radius = 90
    local zoom = 150.0

    -- Filled circle background (horizontal scanlines)
    ...
    -- Crosshairs
    renderDrawLine(cx - radius, cy, cx + radius, cy, 0.5, 0x66FFFFFF)
    renderDrawLine(cx, cy - radius, cx, cy + radius, 0.5, 0x66FFFFFF)

    -- Cardinal direction labels (rotated by player heading)
    local heading = math.rad(getCharHeading(PLAYER_PED))
    local dirs = {{0, "N"}, {math.pi / 2, "E"}, {math.pi, "S"}, {-math.pi / 2, "W"}}
    for _, d in ipairs(dirs) do
        local a = d[1] - heading
        local lx = cx + math.cos(a) * (radius - 14)
        local ly = cy + math.sin(a) * (radius - 14)
        if espFont then
            renderFontDrawText(espFont, d[2], lx - 5, ly - 7, 0xFFFFFFFF)
        end
    end

    -- Player dot (center, blue diamond)
    renderDrawPolygon(cx - 4, cy - 4, 8, 8, 4, 45, 0xFF00BFFF)
    ...
    for id, pos in pairs(moneyBags) do
        ...
        -- Rotate so player's forward is radar-up
        local rx = dx * cos_h + dy * sin_h
        local ry = -dx * sin_h + dy * cos_h
        local scale = radius / zoom
        local dotX = cx + rx * scale
        local dotY = cy - ry * scale
        local dCenter = math.sqrt((dotX - cx)^2 + (dotY - cy)^2)
        if dCenter <= radius then
            local color
            if dist < 100 then color = 0xFF00FF00
            elseif dist < 500 then color = 0xFFFFFF00
            else color = 0xFFFF4444 end
            local size = dist < 100 and 6 or dist < 500 and 5 or 4
            renderDrawPolygon(dotX - size / 2, dotY - size / 2, size, size, 4, 0, color)
        end
        until true
    end
    ...
    -- Title
    if espFont then
        renderFontDrawText(espFont, "RADAR", cx - 18, cy - radius - 14, 0xAAFFFFFF)
    end
end
...
function sampev.onServerMessage(color, text)
    ...
    if keyword and searchTerms then
        ...
        sampAddChatMessage("{FFA500}[Focus]{FFFFFF} ESP locked on: {FFFF00}" .. targetPos.name, 0xFFFFFFFF)
