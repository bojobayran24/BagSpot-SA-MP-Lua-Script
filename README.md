...
## Companion Script: Anti Bike Fall Off

`antifall.lua` applies a safe, automatic patch that prevents the player from falling off bikes.
It auto-patches on game load (no commands needed) and restores original bytes when the script exits.

### Installation
Place `antifall.lua` in your `moonloader/` folder.

---

## Companion Script: Moneybag Hunter (Standalone)

`MoneybagHunter.lua` is a lightweight hunter that scans your saved positions and teleports you to
moneybags efficiently. It uses a smart scan first, then a cluster-skipping scan to avoid redundant
teleports.

### Highlights
- Smart scan: teleports directly if any saved position is within 300m of a tracked moneybag.
- Cluster skipping: after checking a position, it clears nearby positions within 300m.
- Passive alerts: shows readiness in chat when a moneybag is detected nearby.

### Chat Commands
| Command | Description |
| :--- | :--- |
| `/hunt` | Toggle scan on/off |
| `/hunt status` | Show current scan status |
| `/hunt reload` | Reload `SavedPositions.json` |

### Requirements
Uses `config/SavedPositions.json` generated by BagSpot.
